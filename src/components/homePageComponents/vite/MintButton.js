...
const MINTER_ENDPOINT = SERVER+"api/vite/faucet"
...
const mapState = state => ({
  pending_mints: state.vite.pending_mints,
  vite_client: state.global.vite_client,
  vite_name: state.vite.user_name,
  network: state.global.network
});

const mapDispatch = dispatch => ({
  concatPendingMints: newMint => dispatch(viteActions.concatPendingMints(newMint)),
});
...
class MintButton extends Component {
  ...
  handleClick_Mint = async () => {
    ...
    let to = this.props.vite_name
    if (!to) { return; } // user not signed in
	  let post_data = {
	    user: to,
      amount: this.state.amount_to_mint,
    }
  ...
  render() {
    ...
    return (
          <Paper className={classes.paper} elevation={3}>
            ...
            { !vite_name ?
              (
                <Button disabled>Please sign in to get VITED!</Button>
              )
              : (
                <Button
                  ...
                  color="secondary"
                >
                  Mint me {this.state.amount_to_mint} VITED
                </Button>
              )
            }
            ...
            <div>
              ...
              {pending_mints.map((pending_hash, i) => {
                return (<Typography key={i}> 
                  <BloksLogo /> ({i}): 
                  <a
                    href={"https://explorer.vite.net/block/" + pending_hash}
                    target="_blank"
